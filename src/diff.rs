...
pub fn run(args: Vec<String>) -> Result<()> {
    ...
    let workdir = git::require_workdir(&repo, "diff")?;

    if args.is_empty() {
        return git_commands::run_git_interactive(workdir, &["diff"]);
    }
    ...
    for arg in &args {
        if let Some((left, right)) = arg.split_once("..") {
            ...
            let resolved = git::resolve_arg(&repo, arg, &[TargetKind::File, TargetKind::Commit])?;
            match resolved {
    ...
    git_commands::run_git_interactive(workdir, &refs)
}
...
fn resolve_ref_leniently(repo: &git2::Repository, arg: &str) -> String {
    match git::resolve_arg(repo, arg, &[TargetKind::Commit]) {
