...
#[test]
fn update_does_not_force_delete_gone_upstream_branch_with_unique_commits() {
    let test_repo = TestRepo::new_with_remote();
    let remote_path = test_repo.remote_path().unwrap();

    // Create feature-x on the remote and fetch it
    {
        let remote_repo = Repository::open_bare(&remote_path).unwrap();
        let remote_head = remote_repo
            .find_branch("main", BranchType::Local)
    ...
    // Add a local-only commit to feature-x (not pushed)
    test_repo.switch_branch("feature-x");
    test_repo.commit("Local-only work", "local-work.txt");
    test_repo.switch_branch("integration");
    ...
    // Branch with unmerged local commits should NOT be deleted
    assert!(
        test_repo.branch_exists("feature-x"),
        "feature-x should be preserved — it has unmerged local commits"
    );
}
